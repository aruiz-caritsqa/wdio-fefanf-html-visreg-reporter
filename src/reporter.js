...
class VisRegHtmlReporter extends WDIOReporter {
    constructor(opts) {
        ...
        process.on('visreg:comparePass', (images) => {
            console.log(`\n>>> visreg:comparePass :: ${JSON.stringify(images)}\n`);
            let test = this.getTest(this.testUid) ;
            this.moveErrorsToEvents(test) ;
            test.events.push({type: 'imgComparePass', value: { actual: images.actual, baseline: images.baseline }}) ;
        });

        process.on('visreg:compareFail', (images) => {
            console.log(`\n>>> visreg:compareFail :: ${JSON.stringify(images)}\n`);
            let test = this.getTest(this.testUid) ;
            this.moveErrorsToEvents(test) ;
            test.events.push({type: 'imgCompareFail', value: { actual: images.actual, baseline: images.baseline, diff: images.diff }}) ;
        });

        process.on('visreg:compareNew', (images) => {
...
export default VisRegHtmlReporter;
